<?php
namespace WebangonAddon\Widgets;

use Elementor\utils;
use Elementor\Widget_Base;
use Elementor\Controls_Manager;
use Elementor\Group_Control_Typography;
use Elementor\Group_Control_Background;
use Elementor\Group_Control_Box_Shadow;

if (!defined('ABSPATH')) 
    exit;  

class clefix_portfolio_detail_widget extends Widget_Base {

    public function get_name() {
        return 'clenfix-portfolio-detail';
    } 

    public function get_title() {
        return   esc_html__('Portfolio detail', 'clenfix');
    }

    public function get_icon() {
        return 'dashicons dashicons-arrow-down';
    }

    public function get_categories() {
        return array('ashelement-addons');
    }

    protected function register_controls() {

        $this->start_controls_section(
            'section_progress_bar',
            [
                'label' =>   esc_html__('Slider & Info', 'clenfix'),
                'tab' => Controls_Manager::TAB_CONTENT,
            ]
        );

        $this->start_controls_tabs('tctb');

        $this->start_controls_tab(
            'e1',
            [
                'label' => esc_html__( 'Gallery', 'clenfix' ),               
            ]
        );

        $repeater = new \Elementor\Repeater();

        $repeater->add_control(
            'img', [
                'type' => Controls_Manager::MEDIA,
                'default' => [
                    'url' => Utils::get_placeholder_image_src(),
                ],
                'label_block' => true,               
                'label' =>   esc_html__('Image', 'clenfix'),
            ]
        );
        
        $this->add_control(
            'imgs',
            [
                'type' => Controls_Manager::REPEATER,
                'fields' => $repeater->get_controls(),
                'prevent_empty' => false,               
            ]
        );

        $this->end_controls_tab();

        $this->start_controls_tab(
            'e2',
            [
                'label' => esc_html__( 'Info', 'clenfix' ),                
            ]
        );
 
        $this->add_control(
            'title', [
                'type' => Controls_Manager::TEXT,
                'label' =>   esc_html__('Heading', 'clenfix'),
                'label_block' => true,
            ]
        );

        $r = new \Elementor\Repeater();

        $r->add_control(
            'title', [
                'type' => Controls_Manager::TEXTAREA,
                'label' =>   esc_html__('Title', 'clenfix'),
                'label_block' => true,
            ]
        );
        
        $this->add_control(
            'items',
            [
                'type' => Controls_Manager::REPEATER,
                'fields' => $r->get_controls(),
                'prevent_empty' => false,               
            ]
        );

        $this->end_controls_tab();

        $this->end_controls_tabs();

        $this->end_controls_section();

        $this->start_controls_section(
            'detail_section',
            [
                'label' => __('Project Detail', 'clenfix'),
                'tab' => Controls_Manager::TAB_CONTENT,
            ]
        );

        $this->add_control(
            'dtitle', [
                'type' => Controls_Manager::TEXT,
                'label' =>   esc_html__('Heading', 'clenfix'),
                'label_block' => true,
            ]
        );

        $this->add_control(
            'ddesc', [
                'type' => Controls_Manager::TEXTAREA,
                'label' =>   esc_html__('Description', 'clenfix'),
                'label_block' => true,
            ]
        );

        $this->start_controls_tabs('tb');

        $this->start_controls_tab(
            'f1',
            [
                'label' => esc_html__( 'Image', 'clenfix' ),               
            ]
        );

        $this->add_control(
            'img', [
                'type' => Controls_Manager::MEDIA, 
                'default' => [ 
                    'url' => Utils::get_placeholder_image_src(), 
                ],
                'label_block' => true,                
                'label' =>   esc_html__('First image', 'clenfix'), 
            ] 
        ); 

        $this->add_control( 
            'img2', [ 
                'type' => Controls_Manager::MEDIA,
                'default' => [
                    'url' => Utils::get_placeholder_image_src(),
                ],
                'label_block' => true,               
                'label' =>   esc_html__('Second image', 'clenfix'),
            ]
        );

        $this->end_controls_tab();

        $this->start_controls_tab(
            'f2',
            [
                'label' => esc_html__( 'Info', 'clenfix' ),                
            ]
        );

        $this->add_control(
            'stitle', [
                'type' => Controls_Manager::TEXT,
                'label' =>   esc_html__('Heading', 'clenfix'),
                'label_block' => true,
            ]
        );

        $this->add_control(
            'sdesc', [
                'type' => Controls_Manager::TEXTAREA,
                'label' =>   esc_html__('Description', 'clenfix'),
                'label_block' => true,
            ]
        );

        $repeater1 = new \Elementor\Repeater();       

        $repeater1->add_control(
            'txt', [
                'type' => Controls_Manager::TEXT,
                'label_block' => true,
                'label' =>   esc_html__('List', 'clenfix'),
            ]
        );

        $this->add_control(
            'lists',
            [
                'type' => Controls_Manager::REPEATER,
                'fields' => $repeater1->get_controls(), 
                'prevent_empty' => false,
            ]
        );

        $this->end_controls_tab();

        $this->end_controls_tabs();

        $this->end_controls_section();

    }

    protected function render() {
        $settings = $this->get_settings();      
        require dirname(__FILE__) .'/style_1.php';
    }

    protected function content_template() {

    }

}

 $widgets_manager->register_widget_type(new \WebangonAddon\Widgets\clefix_portfolio_detail_widget());